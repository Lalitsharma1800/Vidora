...
const upload_video = asyncHandler(async (req, res) => {
  ...
  if (!title || !uploadId)
    throw new ApiError(400, "Title and uploadId are required");

  const video_file = req.file;

  if (!video_file) throw new ApiError(400, "Video chunk file is required!");

  const video_result = await uploadChunkToCloudinary(
    video_file.buffer,
    upload_id,
  );
  ...
  if (video_result && video_file.done === true) {
    ...
    try {
      ...
      const upload_video = await Video.create(
        [
          {
            videoFile: video_result.secure_url,
            thumbnail: "",
            title: title,
      ...
      const update_video_count = await User.findOneAndUpdate(
        { _id: req.user._id },
        { $inc: { totalVideos: 1 } },
        { returnDocument: "after", session },
      );

      if (!update_video_count)
        throw new ApiError(
          500,
          "video uploading process failed while updating the user's video count",
        );
      ...
      return res
        ...
        .json(
          new ApiResponse(
            200,
            { upload_video, update_video_count },
...
const upload_thumbnail = asyncHandler(async (req, res) => {
  ...
  if (!video_id || !upload_id)
    throw new ApiError(400, "video_id & upload_id is required!!");
  const thumbnail_file = req.file;
  if (!thumbnail_file) throw new ApiError(400, "thumbnail file required!!");
  const thumbnail_result = await upload_Thumbnail_To_Cloudinary(
    thumbnail_file.buffer,
    upload_id,
  );

  if (thumbnail_result && thumbnail_result.done === true) {
    const upload_thumbnail = await Video.findOneAndUpdate(
      { _id: new mongoose.Types.ObjectId(video_id) },
      { $set: { thumbnail: thumbnail_result.secure_url } },
    );
    if (!upload_thumbnail) throw new ApiError(500, "thumbnail upload failed!1");
    return res
      .status(200)
      .json(
        new ApiResponse(200, upload_thumbnail, "thumbnail upload successfully"),
      );
  }
});

export { upload_video, upload_thumbnail };
