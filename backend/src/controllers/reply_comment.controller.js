...
const writeReply = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const write_reply = await Comment_Reply.create(
      [{ commentId: comment._id, userId: user._id, reply: reply }],
      { session },
    );
    if (!write_reply) throw new ApiError("writing reply failed");
    const update_reply_count = await Video.findByIdAndUpdate(
      Video_Comment._id,
      { $inc: { reply: +1 } },
      { session, returnDocument: "after" },
    );
    if (!update_reply_count)
      throw new ApiError("something went wrong while writing the reply");
    ...
    return res
      ...
      .json(
        new ApiResponse(
          200,
          { reply_count: update_reply_count, reply: write_reply },
          "replied successfully commited",
        ),
      );
  } catch (error) {
...
const editReply = asyncHandler(async (req, res) => {
  ...
  if (!mongoose.Types.ObjectId.isValid(replyId))
    throw new ApiError("reply id is invalid");
  ...
  const edit_reply = await Video_Comment.findByIdAndUpdate(
    replyId,
    { reply: edited_reply },
