...
const updateAccountDetail = asyncHandler(async (req, res) => {
  ...
  if (!user)
    throw ApiError(
      500,
      {},
      "There is something went wrong at updateAccountDetail",
    );
...
const updateAvatar = asyncHandler(async (req, res) => {
  ...
  if (!avatar.url)
    throw new ApiError(
      500,
      "Something went wront while uploading the file to cloudinary",
    );
  ...
  if (!user)
    throw new ApiError(500, "avatar did not change please try again later");
...
const updateCoverImage = asyncHandler(async (req, res) => {
  ...
  if (!user)
    throw new ApiError(500, "coverImage did not change please try again later");
