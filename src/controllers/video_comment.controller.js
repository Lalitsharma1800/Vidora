...
const writeComment = asyncHandler(async (req, req) => {
    ...
    try{
        ...
        const write_comment = await Video_Comment.create([{videoId: video._id, userId: user._id, comment: comment}], {session});
        if(!write_comment)  throw new ApiError(500,"writing comment failed");
        const update_comment_count = await Video.findByIdAndUpdate(video._id,{$inc: {comment: +1}}, {session, returnDocument: "after"});
        if(!update_comment_count) throw new ApiError(500,"something went wrong while writing the comment");
        await session.commitTransaction();
        return res
                .status(200)
                .json(new ApiResponse(200, {comment_count: update_comment_count}, "comment successfully commited"));
    }
    catch (error) {
...
const editComment = asyncHandler(async (req, req) => {
    ...
    const edit_comment = await Video_Comment.findByIdAndUpdate(commentId,{comment: newComment},{returnDocument: "after"});
