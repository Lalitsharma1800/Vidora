...
export const saveInHistory = asyncHandler(async (req, res) => {
  ...
  let history = await WatchHistory.findOneAndUpdate(
    { videoId: new mongoose.Types.ObjectId(videoId), userId: req.user._id },
    { $set: { progress: progress, lastWatchedAt: new Date() } },
  ...
  if (!history)
    throw new ApiError(
      500,
      "Something went wrong while saving the video in history",
    );
