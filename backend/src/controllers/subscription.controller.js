...
const toggle_subscribe = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (channel._id.equals(req.user._id))
      throw new ApiError(400, "Own channel cannot subscibed");
    ...
    if (isSubscribed) {
      toggle_subscription = await Subscription.findByIdAndDelete(
        isSubscribed._id,
      ).session(session);
      if (!toggle_subscription)
    ...
    toggle_subscription = await Subscription.create(
      [{ channel: channel._id, subscriber: req.user._id }],
      { session },
    );
    if (!toggle_subscription)
      throw new ApiError(500, "Something went wrong while subscribing").session(
        session,
      );
    await session.commitTransaction();
    return res
...
const getSubscriberList = asyncHandler(async (req, res) => {
  ...
  const subscriberList = await Subscription.find({ channel: user._id }).select(
    "-channel",
  );
...
const getSubscriberCountAndSubscribedStatus = asyncHandler(async (req, res) => {
  ...
  if (!mongoose.Types.ObjectId.isValid(channelId))
    throw new ApiError(400, "please enter select a channel");
  ...
  return res
    ...
    .json(
      new ApiResponse(
        ...
        "subscriberCount & subscribedStatus fetched successfully",
      ),
    );
});
...
export {
  ...
  getSubscriberList,
  getSubscribedToList,
  getSubscriberCountAndSubscribedStatus,
};
